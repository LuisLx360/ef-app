...
interface MatrizEvaluacionOperadorDto {
  ...
  resultadoFinal: number | string; // ✅ CAMBIADO: puede ser string "-"
  respuestas: Record<number, boolean>;
  evaluador?: string; // ✅ NUEVO: Para detectar autoevaluación
}
...
const getResultadoFinalDisplay = (resultadoFinal: number | string, evaluador?: string): string => {
  // Si ya es "-", mantenerlo
  if (typeof resultadoFinal === 'string') return resultadoFinal;
  
  // Si es autoevaluación, mostrar "-"
  if (evaluador && evaluador.toLowerCase().includes('autoevaluación')) {
    return '-';
  }
              ...
              <tbody className="divide-y divide-gray-200">
                {matriz.operadores.map((op, idx) => (
                  <tr key={op.idEvaluacion} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    {/* ✅ COLUMNA INICIAL - AZUL */}
                    <td className="p-4 text-center">
                      <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-bold border border-blue-100">
                        {op.resultadoInicial}%
                      </span>
                    </td>
                    {/* ✅ COLUMNA FINAL - CON LÓGICA CONDICIONAL */}
                    <td className="p-4 text-center border-l">
                      <span className={`px-2 py-1 rounded text-xs font-bold border ${
                        typeof op.resultadoFinal === 'string' || 
                        (op.evaluador && op.evaluador.toLowerCase().includes('autoevaluación'))
                          ? 'bg-gray-100 text-gray-700 border-gray-200'  // Gris para "-"
                          : 'bg-green-50 text-green-700 border-green-100' // Verde para porcentaje
                      }`}>
                        {typeof op.resultadoFinal === 'string' || 
                         (op.evaluador && op.evaluador.toLowerCase().includes('autoevaluación'))
                          ? '-' 
