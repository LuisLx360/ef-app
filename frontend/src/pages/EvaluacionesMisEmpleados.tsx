// src/pages/EvaluacionesMisEmpleados.tsx
...
export default function EvaluacionesMisEmpleados() {
  ...
  useEffect(() => {
    const fetchEvaluations = async () => {
      try {
        ...
        const data: any[] = await apiFetch("/evaluaciones/mis-empleados");

        if (Array.isArray(data)) {
          setEvaluations(
            data.map(api => {
              // Convertimos los datos de "mis empleados" a algo compatible con mapEvaluation
              const apiCompatible: EvaluationApi & { nombreEmpleado?: string } = {
                idEvaluacion: api.idEvaluacion,
                fechaEvaluacion: api.fechaEvaluacion,
                categoria: api.categoria ?? 'Sin categoría',
                estado: api.estado ?? 'pendiente', // default por seguridad
                nombreEmpleado: api.nombreEmpleado,
              };

              return mapEvaluation(apiCompatible);
            })
          );
        } else {
          setEvaluations([]);
        ...
        console.error("❌ Error fetch mis empleados:", err);
        setEvaluations([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Visualiza las evaluaciones de los empleados bajo tu supervisión.
          </p>
        </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            <div className="max-w-sm mx-auto">
              <h3 className="text-gray-900 mb-2 text-lg font-bold">
                No hay evaluaciones de tu equipo
              </h3>
